...
| R-CLOUD-01 | critical | Cloud creation causes uncontrolled cost or exposure | zero-resource default, exact confirmation, independent approvals, four-hour cost envelope and zero billable actions | billing owner, real budget alerts, current price review, IAM/state ownership and explicit spend approval |
| R-CLOUD-02 | critical | USD 5 or a budget alert is mistaken for a hard cap | docs and Terraform outputs state estimates/alerts are planning controls; persistent selected tier rejected | human stop/teardown owner and actual billing observation remain required |
