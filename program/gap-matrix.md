...
| WS-08 Integration Contracts | partial | 17 schemas/ejemplos, OpenAPI 3.1.1 y providers seguros/idempotentes para `EvidenceBundle`, `ProcedureWorkflow`, `ProcedureAssessment`, `EvidenceGapRequest`, `ClaimPack` y lifecycle; EvidenceGap conserva intake inmutable `open`/unverified | Consumers vecinos, lifecycle de resolución de gaps, revocation/supersession cross-product y pruebas entre repos | Provider/consumer contract tests; timeout/retry; boundary; refs/versiones preservadas | P0 | WS-01, WS-05, WS-07 |
...
| WS-11 Quality, Evals and Docs | partial | 667/669 tests detached, 17/17 contratos, 8/8 domain eval y 14 familias nombradas verdes; EvidenceGap 14/14; clean PostgreSQL four-provider smokes y Feature 060 gates pasan | EVAL-SOURCE/MISSING/RBAC/INGEST/CASE/ACCESSIBILITY/RESTORE dedicados; corpus/consumers/browser/a11y/load/restore | Evals faltantes; corpus real; consumers; browser/a11y/load/restore | P0 | Todos |
...
| POST /api/v1/evidence-gap-requests | implemented_with_limits | Intake inmutable tenant-scoped, replay/dedupe/conflicts, anti-authority, audit mínimo, FORCE RLS, OpenAPI y CI remoto pasan; faltan consumer, resolución, retention y operación desplegada |

Autenticación, tenant scope, RBAC, validación, idempotencia, audit y rate limit están implementados para `POST /api/v1/procedure-queries`, `POST /api/v1/evidence-gap-requests`, `POST /api/v1/claim-packs`, workflow lifecycle y la familia operacional de ingestion jobs. Siguen ausentes transversalmente en el catálogo mínimo restante; las rutas pre-v1 son 404 por defecto en producción porque usan queries globales/wildcard CORS.
...
| EVAL-EVIDENCE-GAP-001 | passed_for_immutable_open_intake_with_consumer_privacy_and_operations_limitations | 14/14 prueba auth/RBAC/tenant, exact/concurrent replay, aggregate conflict, anti-authority, audit minimization, FORCE RLS y compiled PostgreSQL HTTP; faltan research resolution, retention, consumer, staging/load/restore y deployment |
...
4. P0 — extender providers a documents/procedures/cases y consumers externos; ProcedureAssessment y EvidenceGap ya están implementados/published/CI-green, pero faltan consumer y resolution lifecycle;
...
7. P0 — mantener las catorce familias verdes, implementar los siete evals dedicados faltantes y ampliar regression a corpus real, consumers, browser, a11y, restore y load;
