...
| **Integration Gateway** | APIs v1, validación de schemas, idempotency, correlation, provenance, adapters y contract tests. | `POST /api/v1/procedure-queries`, `POST /api/v1/claim-packs` y `POST/GET /api/v1/ingestion-jobs`, doce schemas/ejemplos, OpenAPI canónico, idempotency y providers locales están probados. Faltan consumidores externos autenticados, adapters restantes, el gate PostgreSQL remoto de ClaimPack y worker/storage/scanner desplegados. | parcial |
