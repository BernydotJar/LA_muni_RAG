...
| Browser -> human BFF | state, authorization code, cookies, Origin, CSRF/bootstrap proof | state plus HttpOnly browser binding, nonce, PKCE S256, single-use code, rotating opaque session, exact Origin, CSRF, no Bearer | Feature 077 local HTTP, non-owner PostgreSQL, and compiled smoke gates pass; productive IdP adapter, reverse-proxy topology and recovery/MFA are absent |
