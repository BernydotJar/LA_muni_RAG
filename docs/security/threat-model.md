...
| Browser -> human BFF | state, authorization code, cookies, Origin, CSRF/bootstrap proof | state plus HttpOnly browser binding, nonce, PKCE S256, single-use code, rotating opaque session, exact Origin, CSRF, no Bearer | Features 077 and 083 local HTTP, OIDC protocol and non-owner PostgreSQL gates pass; productive provider approval/registration, reverse-proxy topology and recovery/MFA are absent |
| Browser -> role-aware shell | hash route, BFF JSON, keyboard input | same-origin POST lifecycle, closed human-role/permission allowlists, denied-route fallback, no Web Storage/readable cookie, strict CSP/frame/cache/cross-origin headers | Feature 078 HTTP/static and deterministic Chromium viewer/admin gates pass; complete module workflows, real deployment, external users and human accessibility review are absent |
| Human BFF -> telemetry exporter | closed operation/method/outcome/status/duration event | no-op default, exact five-field validator, no identity/session/network/error labels, exporter/clock isolation | Feature 079 local collector and failure-injection gates pass; productive exporter, access, retention, alerts and incident ownership are absent |
...
| Spoofing | Browser callback is bound to an attacker session or external role/tenant claim | state plus HttpOnly browser binding, nonce, PKCE S256, local membership-only authorization, ambiguous membership denial | local deterministic and PostgreSQL gates pass; productive OIDC token/discovery/JWKS validation and provider policy remain human-gated |
...
| Tampering | Replayed authorization code, replaced session cookie, or forged CSRF value | provider-scoped code digest claim, atomic state consumption, token generation/rotation/revocation, exact Origin and session-bound proof | adversarial HTTP and SQL gates pass; concurrency/load, reverse-proxy origin handling and productive provider retries remain unproved |
...
| Information disclosure | Human tokens, codes, cookies, issuer/subject or profile claims enter persistence/audit | one-way digests, protected nonce/verifier, closed audit fields, bounded pre-tenant aggregates, generic errors | local leakage tests and SQL inspection pass; productive telemetry/APM, provider terms and retention controls remain absent |
| Information disclosure | Observability labels reconstruct a user, tenant or session | exact low-cardinality telemetry schema excludes all IDs, URLs, network/client attributes and error text | local serialization/EVAL gates pass; any productive exporter must preserve schema and privacy controls |
