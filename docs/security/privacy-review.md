...
| Role-aware shell state | tenant/principal/session IDs, local roles/permissions, generation and CSRF value in browser memory | render authorized navigation and session lifecycle | Feature 078 deterministic local shell only; no Web Storage, readable cookie, URL, console or public Pages persistence; complete module data flows and productive privacy notice remain absent |
| Human-session route telemetry | closed operation, method class, outcome class, HTTP status, bounded monotonic duration | local regression and aggregate reliability analysis | no tenant/principal/subject/session/request/role/permission/URL/network/client/error dimensions; default exporter is no-op; productive exporter and retention are unapproved |
...
- The role-aware shell may keep the bounded CSRF value only in JavaScript memory for the active page. It must not write identity/session material to Web Storage, IndexedDB, URLs, DOM text, console output, analytics, or the public Pages artifact.
- Human-session telemetry is route-level and aggregate-only. It must not add tenant, principal, session, request, subject, provider, URL, IP, user-agent, body, query or free-form error labels. Any productive exporter requires separate access, retention, deletion and incident-review approval.
...
| Human-session telemetry | no productive exporter or retained dataset exists; local collector is process-only | exporter destination, sampling, access, retention, deletion, backup and legal-hold policy before deployment |
