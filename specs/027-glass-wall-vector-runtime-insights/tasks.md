# Tasks — 027-glass-wall-vector-runtime-insights

## Implementation

- [x] Add requirements and design specs.
- [x] Register feature 027 in `feature_list.json`.
- [x] Add vector insight side panel to `public/glass-wall.html`.
- [x] Enrich vector, embedding, and vector store node copy.
- [x] Add CSS-only graph scan, edge flow, and vector pulse animation.
- [x] Preserve endpoint allowlist.
- [x] Preserve safety contract.
- [x] Preserve reduced-motion protection.
- [x] Update regression tests.
- [x] Update `progress/current.md`.

## Verification

- [ ] `npm run typecheck`
- [ ] `npm run build`
- [ ] `npm run test`

## Manual Review

- [ ] `/glass-wall.html` shows `Vector / Embedding` panel.
- [ ] Vector route has richer explanatory status.
- [ ] Embedding and vector store nodes no longer feel vague.
- [ ] Active and degraded edges have more motion.
- [ ] Reduced motion disables decorative animation.
- [ ] No backend behavior changed.
